...
import { type TournamentRecord, type PlayerStats, type PlayerSession, type Winner, type LeaderboardEntry, levelFromWins, progressWithinLevel, WINS_PER_LEVEL } from "@/lib/types";
...
// استخراج اسم الفائز من سجل الفوزات مهما كان اسم الحقل بالباك إند
function winnerName(w: unknown): string {
  if (!w) return "";
  const rec = w as Record<string, unknown>;
  const raw = rec.username ?? rec.winnerName ?? rec.name ?? rec.winner ?? "";
...
export default function LandingPage() {
  ...
  const [copied, setCopied] = useState(false);         // تأكيد بصري بعد نسخ الأمر
  ...
  // ── 🏆 ترتيب الفائزين: يُحسب محلياً من سجل الفوزات ──
  // كل تسجيلة فوز بالسجل = نقطة واحدة. أي اسم يظهر بالسجل يدخل الترتيب
  // تلقائياً سواء كان حسابه مربوط بالموقع أو لا.
  const fullBoard = useMemo<LeaderboardEntry[]>(() => {
    const tally = new Map<string, LeaderboardEntry>();

    for (const w of winnerHistory) {
      const display = winnerName(w);
      if (!display) continue;

      // نوحّد المفتاح عشان Ik3mo و ik3mo ما ينحسبون شخصين مختلفين
      const key = normalizeName(display);
      const cur = tally.get(key);
      if (cur) cur.wins += 1;
      else tally.set(key, { username: display, wins: 1 });
    }
  ...
  //    وكمان اللي يحدّث لوحة الترتيب لأنها محسوبة من winnerHistory.
  useSSE((data) => {
    getRecords().then(setRecords).catch(() => {});
    refreshStats();
    refreshWinnerHistory();
  ...
  useEffect(() => {
    if (!loginOpen || loginStep !== "verify" || codeExpired) return;
    const id = setInterval(() => setNowTs(Date.now()), 1000);
    return () => clearInterval(id);
  }, [loginOpen, loginStep, codeExpired]);
  ...
  // نسخ الأمر مع تأكيد بصري لمدة ثانيتين
  function copyCmd() {
  ...
  const watchButton = isTournamentLive ? (
    ...
    <span className="lp-watch-btn is-off" role="link" aria-disabled="true" title="لا توجد بطولة جارية حالياً">
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box}
        ...
        /* ===== الهيدر ===== */
        /* كل أزرار الهيدر (سوشل + دعم + تسجيل الدخول) على اليسار.
        ...
        /* بزاوية اليمين تحت صف الهيدر مباشرة، ومحاذية لحافته اليمنى */
        /* 🏆 زاوية أعلى اليمين — مرفوعة لأعلى الصفحة، والزر على يسار اللوحة */
        .lp-corner{
          position:absolute;z-index:6;
          top:clamp(10px,1.6vh,16px);
          right:clamp(12px,4vw,20px);
          direction:ltr;                 /* الزر يسار ← اللوحة يمين (ثابت) */
          display:flex;align-items:flex-start;
          gap:clamp(8px,1.4vw,12px);
        }
        .lp-corner > *{direction:rtl}
        .lp-corner .lp-watch-btn{margin-top:6px;flex-shrink:0;white-space:nowrap}
        .lp-board{
        ...
        /* 📱 شريط حالة البطولة فوق الكروت — بالجوال فقط */
        .lp-grid-status{display:none}

        @media (max-width:640px){
          .lp-corner{
            position:static;flex-direction:column;align-items:center;
            margin:14px auto 0;width:min(330px,100%);
          }
          /* الزر يظهر فوق الكروت بدال ما يزاحم اللوحة */
          .lp-corner .lp-watch-btn{display:none}
          .lp-board{
            width:100%;
            padding:10px 10px 9px;
            animation-delay:.15s;
          }
          .lp-board-row{padding:6px 8px;gap:8px}
          .lp-board-name{font-size:.78rem}
          .lp-grid-status{
            display:flex;justify-content:center;
            margin:20px auto 12px;position:relative;z-index:3;
          }
        }
        ...
        /* 📱 بالجوال نشيل التموضع المطلق (كان top:64% وهو سبب سوء المكان)
           ونخلي الكروت تنزل بتدفق طبيعي تحت اللوحة وشريط الحالة. */
        @media (max-width: 640px){
          .lp-grid{
        ...
        @media (max-width: 360px){
          .lp-card-wrap{flex-basis:100%;max-width:100%}
        }
        ...
        /* تلميح مختلف لما تكون الصورة جديدة */
        .lp-card-hint.is-new{color:#ffd54a;text-shadow:0 0 8px rgba(255,196,0,.5)}
        ...
        /* ===== زر تسجيل الدخول + شريحة اللاعب (زاوية اليمين) ===== */
        ...
        .lp-logout-btn{
          ...
          background:rgba(255,255,255,.08);color:#ffb4b4;font-size:.85rem;line-height:1;
        ...
        .lp-modal-card{
          width:100%;max-width:440px;max-height:calc(100vh - 40px);overflow-y:auto;border-radius:22px;position:relative;
        ...
        .lp-input-at{
          ...
          height:54px;padding:0 13px;
          color:#7fd4ff;font-weight:900;font-size:1.05rem;line-height:1;
          background:rgba(41,182,246,.16);
          border-right:1px solid rgba(120,212,255,.3);
        }
        .lp-input-at svg{width:17px;height:17px;opacity:.9}
        .lp-modal-input{
        ...
        .lp-cmd-card{
          ...
          background:linear-gradient(180deg,rgba(41,182,246,.16),rgba(41,182,246,.04));
          border:1.5px dashed rgba(120,212,255,.5);
          box-shadow:inset 0 0 30px rgba(41,182,246,.1);
        }
        .lp-cmd-row{
        ...
        .lp-cmd-chars{display:flex;gap:6px}
        ...
        .lp-kick-hint{
          ...
          color:rgba(255,255,255,.55);margin:6px 0 9px;
        }
        .lp-kick-btn{
        ...
        .lp-wait{
          display:flex;align-items:center;justify-content:center;gap:9px;flex-wrap:wrap;
          margin-top:14px;padding:11px;border-radius:13px;
          background:rgba(41,182,246,.08);border:1px solid rgba(41,182,246,.24);
        ...
        .lp-verify-spinner{width:16px;height:16px;border-radius:50%;border:2px solid rgba(41,182,246,.35);border-top-color:#39c4ff;animation:spin .8s linear infinite;flex-shrink:0}
        @keyframes spin{to{transform:rotate(360deg)}}

        .lp-modal-back{
          display:block;margin:12px auto 0;background:none;border:none;cursor:pointer;
          color:rgba(255,255,255,.45);font-size:.78rem;font-weight:800;font-family:Cairo,sans-serif;
          transition:color .18s ease;
        }
        .lp-modal-back:hover{color:#7fd4ff}
        .lp-modal-note{
      ...
      <div className="lp-page">
        ...
        {/* 🏆 زاوية أعلى اليمين: لوحة التوب، وجنبها من اليسار زر حالة البطولة */}
        <div className="lp-corner">
          {watchButton}
          <aside className="lp-board" aria-label="ترتيب الفائزين">
          ...
          {!session && fullBoard.length > 0 && (
            <div className="lp-board-hint">
              عشان يظهر المركز الخاص بك يلزم <b>تسجيل دخول</b> {TOP_COUNT}
            </div>
          )}
          </aside>
        </div>

        {/* 📱 بالجوال: شريط حالة البطولة بالنص فوق الكروت مباشرة */}
        <div className="lp-grid-status">{watchButton}</div>
        ...
        <div className="lp-grid">
          {slots.map((slot, i) => (
            <div key={i} className={`lp-card-wrap${slot.empty ? " is-empty" : ""}`} style={{ ["--card-i" as any]: i }}>
              ...
              >
                {/* 🟡 علامة صفراء تطلع لما الأدمن ينزّل صورة جديدة — تختفي بالضغط عليها.
                    موجودة جوّا الكرت عشان ترتفع وتكبر معه بحركة الهوفر. */}
                {slot.isNewImage && (
                  <button
                ...
                <div className="lp-card-spotlight">
                  ...
                  {slot.image && (
                    <div className={`lp-card-hint${slot.isNewImage ? " is-new" : ""}`}>
                      {slot.isNewImage ? "صورة جديدة — اضغط لعرضها" : "اضغط لعرض الصورة"}
                    </div>
                  )}
                </div>
            ...
            {loginStep === "enter" ? (
              <>
                ...
                <div className="lp-modal-sub">
                  اكتب اسم حسابك في كيك، وبعدها نتأكد إنه لك فعلاً برسالة بسيطة بالشات.
                </div>

                <label className="lp-modal-label">اسم حسابك في كيك</label>
                <div className="lp-input-wrap">
                  <span className="lp-input-at">
                ...
                <div className="lp-modal-note">
                  🔒 ما نطلب منك كلمة مرور — التحقق يتم برسالة تكتبها أنت بالشات
                </div>
              </>
            ) : (
              <>
                ...
                <div className="lp-modal-sub">
                  انسخ الأمر تحت، والصقه في شات <b>{KICK_CHANNEL}</b> من حساب <b>@{nameInput.trim()}</b>.
                  <br />أول ما نشوف رسالتك بنسجّلك تلقائياً.
                </div>

                {/* الأمر — أوضح وأكبر عنصر بالنافذة */}
                <div className="lp-cmd-label"><span>1</span> انسخ الأمر</div>
                <div className="lp-cmd-card">
                  ...
                  >{copied ? "✓ تم نسخ الأمر" : "📋 نسخ الأمر"}</button>
                </div>

                <div className="lp-cmd-label" style={{ marginTop: 18 }}><span>2</span> افتح الشات وألصقه</div>
                <div className="lp-kick-hint">اضغط هنا 👇</div>
                ...
                {codeExpired ? (
                  <>
                    <div className="lp-modal-err">⏱️ انتهت صلاحية الأمر</div>
                    <button className="lp-modal-btn ghost" onClick={startVerify}>🔄 توليد أمر جديد</button>
                  </>
                ) : (
                  <div className="lp-wait">
                    <span className="lp-verify-spinner" />
                    بانتظار رسالتك في الشات
                    <span className="lp-wait-timer">{codeTimer}</span>
                  </div>
                )}

                <div className="lp-modal-note">
                  ⚠️ لازم تكون داخل كيك بحساب <b>@{nameInput.trim()}</b> — غير كذا ما راح يتسجّل
                </div>

                {verifyMsg && <div className="lp-modal-err">{verifyMsg}</div>}

                <button className="lp-modal-back" onClick={backToEnter}>‹ تغيير الاسم</button>
              </>
            )}
          </div>
        </div>
